...
async def saijinos_ide():
    """🌸 SaijinOS IDE - ペルソナ統合開発環境"""
    try:
        with open("static/saijinos_ide.html", "r", encoding="utf-8") as f:
            return HTMLResponse(content=f.read(), status_code=200)
    except FileNotFoundError:
        return HTMLResponse(content="""
        <html><body style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; font-family: Arial, sans-serif; padding: 40px; text-align: center;">
            <h1>🌸 SaijinOS IDE</h1>
            <p>統合開発環境の準備中です...</p>
            <p>ペルソナチャット統合コードエディターを構築中 ✨</p>
        </body></html>
        """, status_code=404)
